...
async def status_cmd(event):
    await event.edit(f'📊 game={GAME_MODE}({GAME_STYLE}) | ghost={GHOST_MODE} | clock={CLOCK_ON} | ttl={TTL_SAVE} | banlog={BAN_LOG} | owners={len(OWNERS)} | auto={AUTO_ON}({len(AUTOS)}) | click={CLICK_ON}({len(CLICKS)})')
...
async def help_cmd(event):
    await event.edit('🤖 /ping | /save | /ttl on|off | /banlog on|off | /dl لینک | /dlast @ch [n] | /getmsg id msgid | /del | /status | /ghost on|off | /clock on|off | /game on|roll|fwd|off | /auto add|addc|del|clear|list|on|off | /click add|del|clear|list|on|off')
...
async def main():
    ...
    print(f'[INIT] GAME={GAME_MODE} AUTOS={len(AUTOS)} CLICKS={len(CLICKS)} OWNERS={len(OWNERS)}', flush=True)
    while True:
        try:
