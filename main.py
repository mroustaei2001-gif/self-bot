...
async def auto_cmd(event):
    ...
    if act == 'list':
        ...
        for k, v in AUTOS.items():
            ...
            lines.append(f'• {v.get("title", k)} → هر {mins} دقیقه: {kind}')
    ...
    if act == 'add':
        ...
        if not word:
            ...
            return
        if len(AUTOS) >= 10:
            await event.edit('❌ سقف ۰ گروه (محافظت از اکانت).')
            return
        try:
            ent = await client.get_entity(target)
        except Exception:
            await event.edit('❌ گروه پیدا نشد؛ مطمئن شو عضوشی و آدرس درست است.')
            return
        AUTOS[str(ent.id)] = {'mode': 'txt', 'word': word, 'interval': minutes * 60, 'last': 0, 'title': getattr(ent, 'title', target)}
        save_autos()
        await event.edit(f'✅ هر {minutes} دقیقه «{word}» به‌صورت پیام عادی در {getattr(ent, "title", target)} ارسال می‌شود.')
        return
    if act == 'addc':
        ...
        try:
            ...
            await event.edit('❌ گروه پیدا نشد؛ مطمئن شو عضوشی و آدرس درست است.')
            return
        AUTOS[str(ent.id)] = {'mode': 'copy', 'mid': rmsg.id, 'interval': minutes * 60, 'last': 0, 'title': getattr(ent, 'title', target)}
        save_autos()
        await event.edit(f'✅ هر {minutes} دقیقه کپی آن پیام به‌صورت پیام عادی در {getattr(ent, "title", target)} ارسال می‌شود.')
        return
    if act == 'del':
        try:
            ent = await client.get_entity(rest)
            key = str(ent.id)
        except Exception:
            key = rest
...
async def help_cmd(event):
    await event.edit('🤖 /ping | /save | /del | /status | /ghost on|off | /clock on|off | /game on|roll|fwd|off | /auto add|addc|del|list|on|off')
