        ...
        for cid in list(WATCH):
            try:
                ...
                if isinstance(part, types.ChannelParticipantBanned):
                    ...
                    print(f'[BAN] probe found ban in {cid} by {kicked}', flush=True)
                    await log_ban(cid, kicked, None)
                    WATCH.discard(cid)
            except errors.UserNotParticipantError:
                WATCH.discard(cid)
            except (errors.ChannelPrivateError, errors.ChatPrivateError):
                WATCH.discard(cid)
...
async def status_cmd(event):
    await event.edit(f'📊 game={GAME_MODE}({GAME_STYLE}) | ghost={GHOST_MODE} | clock={CLOCK_ON} | ttl={TTL_SAVE} | banlog={BAN_LOG} | watch={len(WATCH)} | auto={AUTO_ON}({len(AUTOS)}) | click={CLICK_ON}({len(CLICKS)})')
...
async def main():
    ...
    print(f'[INIT] GAME={GAME_MODE} WATCH={len(WATCH)} OWNERS={len(OWNERS)}', flush=True)
    while True:
        try:
