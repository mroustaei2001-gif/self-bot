...
async def auto_cmd(event):
    ...
    if act == 'clear':
        ...
        AUTOS.clear()
        save_autos()
        await event.edit(f'🧹 کل لیست پاک شد ({n} گروه حذف شد).')
        return
    if act == 'list':
        if not AUTOS:
...
async def help_cmd(event):
    await event.edit('🤖 /ping | /save | /del | /status | /ghost on|off | /clock on|off | /game on|roll|fwd|off | /auto add|addc|del|clear|list|on|off')
