...
CLOCK_RE = re.compile(r'[⁰¹²³⁴۵۶۷۸۹ː\s]+$')
    ...
    try:
        ...
        for i in range(12):
            ...
            try:
                ...
                print(f'[GAME] floodwait send {e.seconds}s', flush=True)
                await asyncio.sleep(e.seconds + 1)
                continue
            val = msg.dice.value
...
async def gen_mode():
    ...
    if not CODE:
        ...
        with open(SESSION_PATH + '.hash') as f:
            h = f.read().strip()
        await client.sign_in(PHONE, CODE, phone_code_hash=h, password=PASS2 or None)
...
async def main():
    ...
    try:
        ...
        if not await client.is_user_authorized():
            print('[FATAL] لاگین نیست! یک بار GEN=1 بگذار و دیپلوی کن.', flush=True)
            return
