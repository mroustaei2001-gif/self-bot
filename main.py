...
async def getid_cmd(event):
    ...
    if rmsg:
        fwd = getattr(rmsg, 'forward', None)
        if fwd is not None and getattr(fwd, 'chat_id', None):
            chat_id = fwd.chat_id
        elif rmsg.chat_id != event.chat_id:
    ...
    if chat_id is None:
        async for dialog in client.iter_dialogs(limit=30):
            if dialog.is_group:
                ...
                break
    if chat_id is None:
        await event.edit('❌ گروهی پیدا نشد. یک پیام در گروه بفرست و دوباره امتحان کن.')
        return
    try:
