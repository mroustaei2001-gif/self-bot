...
@client.on(events.NewMessage(pattern=r'^/dl$', chats='me'))
async def dl_cmd(event):
    """دانلود رسانه از پیام reply شده (حتی از کانال‌های قفل‌دار)"""
    rmsg = await event.get_reply_message()
    if not rmsg:
        await event.edit('❌ روی پیام حاوی رسانه reply بزن.')
        return
    if not rmsg.media:
    ...
    try:
        path = await rmsg.download_media()
        await client.send_message('me', file=path, caption=rmsg.text or '')
        await event.delete()
        try:
            os.remove(path)
        except Exception:
...
async def getmsg_cmd(event):
    """دریافت پیام از هر چت با ID و آپلود در Saved Messages"""
...
async def help_cmd(event):
    await event.edit('🤖 /ping | /save | /dl | /getmsg | /del | /status | /ghost on|off | /clock on|off | /game on|roll|fwd|off | /auto add|addc|del|clear|list|on|off | /click add|del|clear|list|on|off')
