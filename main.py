    ...
    try:
        ...
        if path:
            ...
            try:
                ...
                pass
            print(f'[TTL] saved from {name}', flush=True)
    except Exception as e:
...
async def ghost_mirror(event):
    ...
    try:
        ...
        print(f'[MIRROR] mirrored from {name}', flush=True)
    except Exception as e:
...
async def status_cmd(event):
    await event.edit(f'📊 game={GAME_MODE}({GAME_STYLE}) | ghost={GHOST_MODE} | clock={CLOCK_ON} | ttl={TTL_SAVE} | auto={AUTO_ON}({len(AUTOS)}) | click={CLICK_ON}({len(CLICKS)})')
...
async def clock_cmd(event):
    ...
    if event.pattern_match.group(1) == 'on':
        ...
        await event.edit(f'🕐 ساعت زنده روشن شد! (فونت هر ساعت عوض می‌شود)\nنمونه الان: {small_time()}')
    else:
        CLOCK_ON = False
...
async def help_cmd(event):
    await event.edit('🤖 /ping | /save | /ttl on|off | /dl لینک | /dlast @ch [n] | /getmsg id msgid | /del | /status | /ghost on|off | /clock on|off | /game on|roll|fwd|off | /auto add|addc|del|clear|list|on|off | /click add|del|clear|list|on|off')
...
async def main():
    ...
    print(f'[INIT] GAME={GAME_MODE} AUTOS={len(AUTOS)} CLICKS={len(CLICKS)} GHOST={GHOST_MODE} TTL={TTL_SAVE}', flush=True)
    while True:
        try:
