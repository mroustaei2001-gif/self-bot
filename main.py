...
async def game_fix(event):
    ...
    if target is None:
        return
    print(f'[GAME] dice {emoji} in chat {event.chat_id} - stealth mode', flush=True)
    GAME_BUSY = True
    preroll = []
    try:
        try:
        ...
        for i in range(30):
            ...
            if m.dice and m.dice.value == target:
                win = m
                print(f'[GAME] preroll win at try {i}: {target}', flush=True)
                break
            await asyncio.sleep(0.05)
        if win is None:
            win = m
            print('[GAME] preroll no win - forwarding last', flush=True)
        try:
        ...
        SEEN.add(fwd.id)
        print(f'[GAME] forwarded winner to chat', flush=True)
    except Exception as e:
