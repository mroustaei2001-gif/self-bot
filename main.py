...
@client.on(events.NewMessage(pattern=r'^/click\s+(add|addh|del|list|on|off|clear|peek)\s*(.*)$', chats='me'))
async def click_cmd(event):
    ...
    if act == 'peek':
        if LAST_BTN is None:
            await event.edit('❌ هنوز هیچ پیام دکمه‌داری ندیده‌ام. اول در گروه منتظر پیام دکمه‌دار بمان.')
            return
        try:
    ...
    if act == 'addh':
        ...
        save_clicks()
        await event.edit(f'🖱️ از این به بعد دکمه‌های حاوی «{rest}» در {title} خودکار کلیک می‌شوند.')
        return
    if act == 'add':
...
async def help_cmd(event):
    await event.edit('🤖 /ping | /save | /spamcheck | /ttl on|off | /banlog on|off | /dl لینک | /dlast | /getmsg | /del | /status | /ghost | /clock | /game | /auto ... | /click add|addh|peek|del|clear|list|on|off')
